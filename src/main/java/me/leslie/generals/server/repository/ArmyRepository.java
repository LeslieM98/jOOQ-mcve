...
import me.leslie.generals.server.entity.interfaces.IArmy;
...
import me.leslie.generals.server.entity.pojos.Army;
...
public class ArmyRepository {
    ...
    private Set<Army> transformToRelation(IArmyComposition army) {
        return army.getTroops().stream().map(x -> new Army(army.getHQ().getId(), x.getId()))
     ...
     * @param army
     * @return
     * @throws UpdateFailedException if HQ is not present in repo or any of the
     *                               given troops are not present in the repo
     */
    public IArmyComposition updateRelation(IArmyComposition army) {
        Collection<Army> relations = transformToRelation(army);
        Collection<Integer> alreadyExisting = troopRepository
                .get(army.getTroops().stream().map(ITroop::getId).collect(Collectors.toSet())).stream()
                .map(ITroop::getId).collect(Collectors.toSet());

        if (alreadyExisting.contains(army.getHQ().getId())) {
            throw new IllegalStateException("HQ does not exist in Repo");
        }
        if (alreadyExisting.containsAll(relations)) {
            throw new IllegalStateException("Some troops are not existing in Repo");
        }
        ...
        for (Army x : relations) {
            try {
                ...
                throw new IllegalStateException("Could not insert " + x.toString(), e);
            }
        }
        return army;
    }
    ...
    public Optional<IArmyComposition> get(int hqID) {
        ...
        Set<? extends ITroop> troops = new HashSet<>(
                troopRepository.get(relations.stream().map(IArmy::getHq).collect(Collectors.toSet())));
    ...
    public List<? extends IArmyComposition> get() {
        return get(jooq.select(Tables.ARMY.HQ).from(Tables.ARMY).fetch().stream().map(Record1::component1)
                .collect(Collectors.toList()));
    }

    public List<? extends IArmyComposition> get(Collection<Integer> ids) {
        return ids.stream().map(this::get).filter(Optional::isPresent).map(Optional::get).collect(Collectors.toList());
    }
    ...
    public void delete(Collection<Integer> hqIDs) {
        jooq.delete(Tables.ARMY).where(hqIDs.stream().map(Tables.ARMY.HQ::eq).reduce(Condition::or).orElseThrow(() -> {
            throw new IllegalStateException("Could not reduce condition");
        }));
    }
