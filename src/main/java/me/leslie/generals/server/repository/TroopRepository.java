...
public class TroopRepository {
    ...
    private OptionalInt nextID() {
        ...
        Set<Integer> results = jooq.select(TROOP.ID).from(TROOP).fetch().stream().map(Record1::component1)
                .collect(Collectors.toUnmodifiableSet());

        return IntStream.iterate(1, i -> i + 1).dropWhile(results::contains).limit(1).reduce(Integer::sum);
    }
    ...
    public ITroop create(ITroop troop) {
        ...
        if (nextID.isEmpty()) {
            throw new IllegalStateException("Could not fetch next id");
        }
